...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "importprivkey <FlyCoinprivkey> [label]\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <FlyCoinaddress>\n"
            "Reveals the private key corresponding to <FlyCoinaddress>.");
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    std::map<CKeyType, int64_t> mapKeyBirth;

    std::set<CKeyType> setKeyPool;
    ...
    // sort time/key pairs
    std::vector<std::pair<int64_t, CKeyType> > vKeyBirth;
    for (std::map<CKeyType, int64_t>::const_iterator it = mapKeyBirth.begin(); it != mapKeyBirth.end(); it++) {
    ...
    // produce output
    file << strprintf("# Wallet dump created by FlyCoin %s (%s)\n", CLIENT_BUILD.c_str(), CLIENT_DATE.c_str());
